...
export function FIDECalculator({ initialResults, onReset }: FIDECalculatorProps) {
  ...
  useEffect(() => {
    if (initialResults) {
      console.log('FIDECalculator received initialResults:', initialResults);
  ...
  useEffect(() => {
    console.log('FIDECalculator component mounted');
  }, []);

  const handleCalculate = (calculationResult: FideRatingResult) => {
    console.log('Parent component received calculation result:', calculationResult);
    console.log('State before update - results:', results, 'visualizationType:', visualizationType);
    ...
    try {
      ...
      setVisualizationType(prev => prev);
    } catch (err) {
  ...
  useEffect(() => {
    if (results) {
      console.log('Results state updated:', {
        currentRating: results.currentRating,
        newRating: results.newRating,
        ratingChange: results.ratingChange,
        performanceRating: results.type === 'fide' ? results.performanceRating : undefined
      });
    }
  }, [results]);
  ...
  return (
    <div className="w-full">
      ...
      <FIDECalculatorForm onCalculate={handleCalculate} />
